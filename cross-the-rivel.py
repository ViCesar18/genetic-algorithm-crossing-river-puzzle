...
def calculate_avaliation(state: list):
  if state.count(Genes.CROSSING.value) > 2 or \
    (
      state.count(Genes.CROSSING.value) != 0 and 
      state[Person.DAD.value] != Genes.CROSSING.value and
      state[Person.MOM.value] != Genes.CROSSING.value and
      state[Person.POLICEMAN.value] != Genes.CROSSING.value
    ) or \
    (
      state[Person.POLICEMAN.value] != state[Person.THIEF.value] and state.count(state[Person.THIEF.value]) != 1
    ) or \
    (
      state[Person.MOM.value] != state[Person.DAD.value] and
      (state[Person.MOM.value] == state[Person.BOY_1.value] or state[Person.MOM.value] == state[Person.BOY_2.value]) or
      (state[Person.DAD.value] == state[Person.GIRL_1.value] or state[Person.DAD.value] == state[Person.GIRL_2.value])
    ):
    return -1
  else:
...
for j in range(30):
  ...
  print(f'Cruzamentos da {j + 1}º execução: {i}')
  mean.append(i)
